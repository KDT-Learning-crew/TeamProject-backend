const bens = bensList.map(ben => ({
	...
	startDate: ben.benStartDate,
	endDate: ben.benEndDate,
	period: ben.benPeriod
}));
...
let totalPages = Math.ceil(bens.length / banSize);
...
function getban(page) {
  ...
  const banDisplay = bens.slice(start, end);
  banDisplay.forEach(ban => {
    const banItem = document.createElement('li');
    banItem.classList.add('manager-ban-user-list-item');

    const countStartDate = new Date(bens.benStartDate);
	const countPeriod = new Date(bens.benPeriod)
    /*const countEndDate = new Date('2025-01-25');*/
    const endDate = countStartDate + countPeriod;
    banItem.innerHTML = `
      ...
      <div class="manager-ban-user-list-ban-start-date">${ban.benStartDate}</div>
      <div class="manager-ban-user-list-ban-end-date">${endDate}</div>
    ...
    banControllButton.addEventListener('click', () => {
      ...
      if (checkUnBan) {
        const indexToRemove = banUserList.findIndex(user => user.userId === ban.userId);
        if (indexToRemove !== -1) {
          benList.splice(indexToRemove, 1); // 해당 Index의 유저 제거
          totalban = benList.length; // 유저 삭제 후 총 게시물 수 갱신
